...
**Пользователи**  
Размер одной записи - около 200 байт.
...
Итого на пользователя: 200 + 1280 + 480 = 1960 байт

Суммарный размер данных: 500 млн * 1960 байт = **0.98 Тб**  
Размер без связей: 500 млн * 200 байт = 0.1 Тб 

**Гилдии**    
Размер одной записи - около 200 байт.

У гилдии в среднем 5 ролей, роль занимает около 40 байт

У гилдии в среднем по 15 голосовых и текстовых каналов, каждый канал с названием и правами доступа весит, скажем, 100 байт (сильно зависит от того, как мы храним права доступа)

Учитывая, что гильдий всего 25 млн:

25 млн * (200 + 40 * 5 + 100 * 30) = **64.6 Гб**

**Диалоги и групповые чаты**  
Похожи на гильдии, но не имеют ролей и множества голосовых/текстовых каналов  
Размер одной записи - около 100 байт.
...
Сумма: 1.5 млрд * 100 байт = **150 Гб**

**Сообщения**    
Учитывая 4 млрд сообщений в день, 3 трлн кажется разумным суммарным количеством сообщений в Discord.

Одно сообщение весит около 250 байт, 5% сообщений содержат вложения - запись в БД о вложении весит 50 байт

Таким образом, сумма: 3 трлн * (250 + 0.05 * 50) = **757.5 Тб**
