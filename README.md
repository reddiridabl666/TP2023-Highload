...
В каждом датацентре ставим по 2-4 машины с nginx.  
На эти машины балансировка будет производиться при помощи BGP  
Далее nginx проксирует запросы на инстансы Gateway-сервиса по round-robin стратегии.  
Gateway ходит в остальные сервисы (кроме голосового чата).

В случае голосовых чатов пользователи подключаются к голосовым серверам напрямую.  
Адрес пользователи получают следующим образом:  
1. Если это первый участник чата:  
    Выбираем наименее загруженный сервер в регионе с помощью service discovery (например, etcd), прикрепляем его адрес к чату, возвращаем адрес пользователю

2. Если чат уже активен:  
    Возвращаем пользователю адрес сервера, к которому привязан чат

Отказоустойчивость будет обеспечиваться следующим образом:
1. Несколько машин с nginx в каждом датацентре
2. Несколько машин с несколькими инстансами application сервера
3. При падении голосового сервера клиент посылает сообщение на Gateway для присвоения голосовому чату адреса нового сервера
4. Следует выделить железо, способное выдержать 2 * Пиковый трафик, для обеспечения работоспособности сервиса при отказе половины машин
