...
Итого на пользователя: 338 + 1280 + 480 = 2098 байт

Суммарный размер данных: 500 млн * 2098 байт = **1.05 Тб**  
Размер без связей: 500 млн * 338 байт = 0.17 Тб 
...
| Таблица           | Ключ шардирования          | Ключ кластеризации | Кол-во записей, млн |
| ----------------- | :------------------------- | ------------------ | :-----------------: |
| User              | id                         | -                  |         500         |
| ChatByUser        | user_id                    | chat_id            |       30 000        |
| UserByChat        | chat_id                    | user_id            |       30 000        |
| GuildByUser       | user_id                    | guild_id           |       10 000        |
| UserByGuild       | guild_id                   | user_id            |       10 000        |
| Guild             | id                         | -                  |         25          |
| GuildTextChannel  | guild_id                   | text_channel_id    |         375         |
| GuildVoiceChannel | guild_id                   | voice_channel_id   |         375         |
| Message           | text_channel_id, bucket_id | message_id         |      3 000 000      |


### Схема резервного копирования

Для всех таблиц, кроме Messages, можно делать снапшот раз в день.  
Размер Messages - свыше 1.5 Пб, поэтому кажется разумным делать снапшот раз в неделю, а между снапшотами догонять инкрементальными бэкапами.
